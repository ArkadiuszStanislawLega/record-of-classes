...
class _Start2PageState extends State<Start2Page> {
      ...
      floatingActionButton: SpeedDial(
        ...
        children: [
          SpeedDialChild(
            ...
            label: AppStrings.addClassesType,
            backgroundColor: Colors.amberAccent,
            onTap: () {
              Navigator.pushNamed(context, AppUrls.createClassesType,
                  arguments: {AppStrings.function: _createClassesType});
            },
          ),
          SpeedDialChild(
  ...
  void _createClassesType(ClassesType createdClassesType) {
    setState(
      () {
        createdClassesType.addToDb();
        var newNode = TreeNodeData(
            label: createdClassesType.name, object: createdClassesType);

        _controller = TreeViewController();
        for (var element in _elements) {
          _controller.removeItem(element);
        }
  ...
  void _removeDialogWindow(TreeNodeData item) {
      ...
      content = Column(
        ...
        children: [
          Column(
            ...
            children: [
              const Text(
                  '${AppStrings.areYouSureToRemove} ${AppStrings.classes}:'),
              Text(
                classes.name,
    ...
    if (item.object is ClassesType) {
      ...
      content = Text('${AppStrings.areYouSureToRemove} ${classesType.name}');
    }
